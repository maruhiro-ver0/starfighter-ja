...
void alien_setKlineAttackMethod(object *alien)
{
	if (alien->shield <= 500)
	{
		setRadioMessage(FS_KLINE, "もうよい!! これで終わりだ!!!", 1);
	...
	{
		setRadioMessage(FS_KLINE, "あきれるような しぶとさだ! これでも食らえ!!", 1);
		if (game.difficulty == DIFFICULTY_ORIGINAL)
		{
	...
	{
		setRadioMessage(FS_KLINE, "すばらしいぞバンフィールド。少し本気を出してやろう…", 1);
...
void alien_destroy(object *alien, object *attacker)
{
	...
	{
		setRadioMessage(FS_KLINE, "光栄に思うぞ…貴様と戦えたことを…", 1);
		alien->dx = alien->dy = 0;
		alien->shield = -150;
	}
}
	...
	if (alien->classDef == CD_KLINE)
	{
		if (game.area == MISN_ELAMALE)
		{
				...
				!(alien->flags & FL_LEAVESECTOR))
			{
				alien->flags |= FL_LEAVESECTOR;
				alien->flags &= ~FL_CIRCLES;
				setRadioMessage(FS_KLINE, "お前を見くびっていたようだ、バンフィールド。また会うことになるだろう!", 1);
			}
		}
		else if (game.area == MISN_EARTH)
		{
				...
				!(alien->flags & FL_LEAVESECTOR))
			{
				alien->flags |= FL_LEAVESECTOR;
				alien->flags &= ~FL_CIRCLES;
				setRadioMessage(FS_SID, "クリス、ケスランが逃げるぞ!", 1);
			}
		}
